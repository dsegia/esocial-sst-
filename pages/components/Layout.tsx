...
export default function Layout({ children, pagina }: { children: React.ReactNode; pagina: string }) {
  ...
  useEffect(() => {
    supabase.auth.getSession().then(({ data: { session } }) => {
      ...
      supabase.from('usuarios').select('nome, empresas(razao_social)').eq('id', session.user.id).single()
        .then(({ data }) => {
          if (data) {
            setNomeUser((data as any).nome)
            setNomeEmpresa((data as any).empresas?.razao_social || '')
          }
        })
    })
  }, [])
